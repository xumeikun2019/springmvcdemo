...
layui.config({
    ...
    if(window.sessionStorage.getItem('userInfo')){
        ...
        var citys;
        $(".realName").val(userInfo.realName); //用户�?
        $(".userSex input[value="+userInfo.sex+"]").attr("checked","checked"); //性别
        $(".userPhone").val(userInfo.userPhone); //手机�?
        $(".userBirthday").val(userInfo.userBirthday); //出生年月
        $(".userAddress select[name='province']").val(userInfo.province); //�?
        //填充省份信息，同时调取市级信息列�?
    ...
    //判断是否修改过头像，如果修改过则显示修改后的头像，否则显示默认头�?
...
function loadProvince() {
    ...
    form.on('select(province)', function(data) {
        $form.find('select[name=area]').html('<option value="">请�?�择�?/�?</option>');
...
function loadCity(citys) {
    var cityHtml = '<option value="">请�?�择�?</option>';
...
function loadArea(areas) {
    var areaHtml = '<option value="">请�?�择�?/�?</option>';
