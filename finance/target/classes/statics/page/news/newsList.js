layui.config({
	...
	$.get("../../json/newsList.json", function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核文�?"){
	...
	$(".recommend").click(function(){
		...
		if($checkbox.is(":checked")){
			var index = layer.msg('推荐中，请稍�?',{icon: 16,time:false,shade:0.8});
            setTimeout(function(){
                layer.close(index);
				layer.msg("推荐成功");
            },2000);
		}else{
			layer.msg("请�?�择�?要推荐的文章");
		}
	})
	...
	$(".audit_btn").click(function(){
		...
		if($checkbox.is(":checked")){
			var index = layer.msg('审核中，请稍�?',{icon: 16,time:false,shade:0.8});
            setTimeout(function(){
	...
	form.on('switch(isShow)', function(data){
		var index = layer.msg('修改中，请稍�?',{icon: 16,time:false,shade:0.8});
        setTimeout(function(){
            layer.close(index);
			layer.msg("展示状�?�修改成功！");
        },2000);
	...
	$("body").on("click",".news_edit",function(){  //编辑
		layer.alert('您点击了文章编辑按钮，由于是纯静态页面，�?以暂时不存在编辑内容，后期会添加，敬请谅解�?��?��??',{icon:6, title:'文章编辑'});
	})

	$("body").on("click",".news_collect",function(){  //收藏.
		if($(this).text().indexOf("已收�?") > 0){
			layer.msg("取消收藏成功�?");
			$(this).html("<i class='layui-icon'>&#xe600;</i> 收藏");
		}else{
			layer.msg("收藏成功�?");
			$(this).html("<i class='iconfont icon-star'></i> 已收�?");
		}
	})
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td><input type="checkbox" name="show" lay-skin="switch" lay-text="是|�?" lay-filter="isShow"'+currData[i].isShow+'></td>'
