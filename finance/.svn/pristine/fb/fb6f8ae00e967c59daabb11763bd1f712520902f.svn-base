...
public class CPlaceServiceImpl implements CPlaceService {
			...
			for (Map<String, Object> roomMap : roomList) {
				...
				for (Map<String, Object> meetingMap : meetingAllList) {
					...
					if (StringUtils.equals(id, meetingId)) {
						...
						if (StringUtils.isNotBlank(timeStartStr) && StringUtils.isNotBlank(timeEndStr)) {
							String[] timeStartArr = timeStartStr.split(" ")[1].split(":");
							String timeStart = timeStartArr[0]  + ":" + timeStartArr[1];
							String[] timeEndArr = timeEndStr.split(" ")[1].split(":");
							String timeEnd = timeEndArr[0] + ":" + timeEndArr[1];
							meetingMap.put("timeStart", timeStart);
							meetingMap.put("timeEnd", timeEnd);
						}
	...
	@Cache
	@Override
	public JsonResult findCplaceByParentIdORlevel(Map<String, Object> map) {
		JsonResult jsonResult = new JsonResult();
		List<Map<String,Object>> placeList = cPlaceMapper.findCplanceByParentIdORlevel(map);
		jsonResult.setData(placeList);
		return jsonResult;
	}
	 ...
	 */
	@Cache
	public List<Map<String, Object>> findjk() {

		return cPlaceMapper.findjk();
	}
