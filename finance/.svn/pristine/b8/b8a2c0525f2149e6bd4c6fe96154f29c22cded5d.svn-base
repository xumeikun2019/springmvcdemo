...
var table1;
$(function(){
	var url = '${ctx}/main/print/list_yer.do';   
    var columns = [ //定义列
        {data: "name"},
        {data: "dycs"},
        {data: "dyys"},
        {data: "dysj"},
        {data: function (obj) {
          	 return "<a class='layui-btn layui-btn-sm _aa' onclick='newTab(\"年打印统计(人员)\",\"\",\"${ctx}/main/print/index_yer.shtml?depid="+obj.department_id+"&sj="+obj.dysj+"\");'>查看</a>";
       }}
    ];
//    $('table.layui-table thead tr th:eq(0)').addClass('layui-hide');
	...
	var url1 = '${ctx}/main/print/list_yer_dyj.do';   
    var columns1 = [ //定义列
        {data: "cname"},
        {data: "dycs"},
        {data: "dyys"},
...
function close(){
 ...
 function searchGoods(table){
	 ...
	 var param = "?name="+name+"&sj="+sj;
		var url = table.ajax.url("${ctx}/main/print/list_yer.do"+param);
		url.load();
}
 function searchGoods1(table1){
	 ...
	 var sj = $("#sj1").val();
	 var param = "?name="+name+"&sj="+sj;
		var url = table1.ajax.url("${ctx}/main/print/list_yer.do"+param);
		url.load();
}
