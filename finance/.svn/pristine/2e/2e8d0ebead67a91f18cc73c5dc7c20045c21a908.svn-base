...
<body>
		...
		<div class="table_control">
			<div class="table_opera">
				<button type="button" title="添加"
					...
					onclick='newTab("添加考勤机","","${ctx}/main/registration/attendanceAdd.shtml");'>
	...
	<script type="text/javascript">
		...
		var table;
		$(function() {
			var url = '${ctx}/main/device/list.do?type=attendance';
			var columns = [ //定义列
					{"data" : function(obj) {
			...
			//修改设备
			 function edit() {
			    ...
			    if (size == 0) {
			         ...
			         newTab("编辑考勤机","","${ctx}/main/device/editCdevice.do?id=" + id);
			    } else if (size > 1) {
						...
						var options = {
								...
								success:function(data){
									if(data.result=='ok'){
										...
										table.ajax.url("${ctx}/main/device/list.do?type=attendance").load();
									}
									if(data.result=='fail'){
					                    layer.msg('删除失败!', {
					                        icon: 2
