...
public class CDeviceController extends GridController {
    ...
    public String editCamera(String id, ModelMap model) {
    	try {
    		//根据id查询设备
    		CDevice pojo = cDeviceService.findCDeviceListById(id);
    		//构造楼层数据,查询所有楼层
    	...
    	return "main/registration/cameraEdit";
    }
    /**
     * 跳转修改广告机adv
     * @param id
    ...
    public JsonResult findByDevice(String ip) {
    	JsonResult jsonResult = new JsonResult();
    	try {
    		jsonResult = cDeviceService.findByDeviceIp(ip);
		} catch (Exception e) {
			jsonResult.setResult(Constants.OPERATION_FAIL);
			e.printStackTrace();
		}
    ...
    @ResponseBody
   	@RequestMapping(value = "/ajaxAsyn", method = RequestMethod.POST)
       public JsonResult findByDevice(@RequestParam Map<String, Object> map) {
       	JsonResult jsonResult = new JsonResult();
       	try {
       		jsonResult = cDeviceService.findDeviceByMap(map);
   		} catch (Exception e) {
   			jsonResult.setResult(Constants.OPERATION_FAIL);
   			e.printStackTrace();
   		}
    ...
    public JsonResult updateState(@RequestParam Map<String,Object> map) {
    	JsonResult jsonResult = new JsonResult();
    	try {
    		jsonResult = cDeviceService.updateCDevice(map);
		} catch (Exception e) {
			jsonResult.setResult(Constants.OPERATION_FAIL);
			e.printStackTrace();
		}
