		        	...
		        	if (typeof(timeFirst) == 'undefined') {
		      		...
		      		if (sameIndex > secondIndex) {
			        	...
			        	if (clickTime == 1) {
			        		...
			        		var tip = '您当前选择的是' + meetingRoomName + '<br>日期为：' + day + ' ' + startTime + '-' + endTime + '<br>点击确认按钮后进入添加预约信息界面';
...
<body>
	<div class="container">
		...
		<div class="am_con" id="freezings" style=""
			...
			<table class="hysRes" id="hysResTable" border="1"
				...
				<tbody style="height: 80%" id="tablebody">
					<!-- <tr id="meeting-time" data-key="测试会议室" data-value="2019102600659769">
						<td>
						<div> 测试会议室
							</div>
						</td>
		...
		<blockquote class="layui-elem-quote" style="margin-top: 30px;">使用说明：点击空白区域选择会议室时段  每小格为15分钟
			&nbsp;&nbsp;状态  蓝色：选中时段 &nbsp;&nbsp;&nbsp;&nbsp; 绿色：会议审核通过 &nbsp;&nbsp;&nbsp;&nbsp;灰色：会议待审核</blockquote>
	</div>
</body>
</html>