...
public class CAttendanceTimeController extends GridController {
    ...
    public GridJsonResult listJson(@RequestParam Map<String, Object> map, EntyPage page) {
        GridJsonResult result = null;		try {			map.put("page",page);			List<Map<String,Object>> list = cAttendanceTimeService.findCAttendanceTimeList(map).getList();			result = new GridJsonResult(page);			result.setData(list);		} catch (Exception e) {			e.printStackTrace();		}		return result;
    }
    ...
    public String edit(String id, ModelMap model) {
        ...
        try {
        	List<Map<String,Object>> list = cAttendanceTimeService.findGsList(usermap.get("id").toString());
//			model.addAttribute("parentId",list.get(0).get("parent_id"));
//			model.addAttribute("parentName",list.get(0).get("name"));			CAttendanceTime pojo = cAttendanceTimeService.findCAttendanceTimeListById(id);			model.addAttribute("pojo",pojo);		} catch (Exception e) {			e.printStackTrace();		}		return "main/commute/edit";
    }

    @ResponseBody
    @RequestMapping(value= "/save",method = RequestMethod.POST, consumes = "application/json")
    public JsonResult create(@RequestBody CAttendanceTime pojo) {
        JsonResult result = new JsonResult();		try {			 result = cAttendanceTimeService.saveOrUpdate(pojo);		} catch (Exception e) {			result.setResult(Constants.OPERATION_FAIL);			e.printStackTrace();		}		return result;
    }

    @ResponseBody
    @RequestMapping(value= "/del",method = RequestMethod.POST)
    public JsonResult del(String id) {
        JsonResult result = new JsonResult();		try {			 result = cAttendanceTimeService.del(id);		} catch (Exception e) {			result.setResult(Constants.OPERATION_FAIL);			e.printStackTrace();		}		return result;
    }
}