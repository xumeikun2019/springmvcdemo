...
public class CUserServiceImpl implements CUserService {
    ...
    public JsonResult saveOrUpdate(CUser pojo) throws Exception {
		...
		if (pojo != null) {
			...
			//判断卡号是否重复
			if (StringUtils.isNotBlank(pojo.getKh())) {
				int khcf = cUserMapper.findUserkh(pojo.getKh());
				if(khcf > 0) {
					jsonResult.setResult("kh");
					jsonResult.setMessage("卡号重复！！！请重新填写");
					return jsonResult;
				}
			}
			...
			if (StringUtils.isNotBlank(id)) {
				...
				//修改人员的同时，往打卡设备上更新人员信息()
				List<Map<String,Object>> list = sysInfoMapper.findIp();
				add Add=new add();
				for(int i=0;i<list.size();i++) {
					Add.xgry(list.get(i).get("ip").toString(),pojo.getId(), pojo.getRealname(),pojo.getKh());
...
//				add Add=new add();
//				for(int i=0;i<list.size();i++) {
//					Add.tjry(list.get(i).get("ip").toString(),pojo.getId(), pojo.getRealname(),pojo.getKh());
//					if(pojo.getFeature()!=null) {
//						Add.tjryzp(list.get(i).get("ip").toString(),pojo.getId(), pojo.getFeature());
//					}
//				}
 		...
 		if (StringUtils.contains(id,",")) {
 			...
 			if (idsArr.length > 0) {
 				for (String idArr : idsArr) {
 					...
 					add Add=new add();
 					for(int i=0;i<list.size();i++) {
 						Add.scdgry(list.get(i).get("ip").toString(),idArr);
 					}
 					if (result == 0) {
 						break;
 					}
 			...
 			//删除考勤设备上的人员信息
				...
				add Add=new add();
				for(int i=0;i<list.size();i++) {
					Add.scdgry(list.get(i).get("ip").toString(),id);
				}
 		}
 		if (result == 0) {
 			jsonResult.setResult(Constants.OPERATION_FAIL);
