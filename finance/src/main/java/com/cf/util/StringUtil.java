...
import com.cf.project.SystemSetting;
...
//		//System.out.println("getRoot path is "+filePath );
	 ...
	 * @return
	 */
	public static String hexHighToLow(String hexStr) {
		String result = "0";
		if (StringUtils.isNotBlank(hexStr) && StringUtils.length(hexStr) == 4) {
			String one = StringUtils.substring(hexStr, 0,2);
			String two = StringUtils.substring(hexStr, 2,4);
			int partOne = Integer.parseInt(one, 16);
			int partTwo = Integer.parseInt(two, 16);
			if (partOne < partTwo) {
				result = String.valueOf(one) + String.valueOf(two);
			} else {
				result = String.valueOf(two) + String.valueOf(one);
			}
		}
		return result;
	}
	 ...
	 * @return
	 */
	public static String weekConvert(int weekInt) {
		String week = StringUtils.EMPTY;
        switch(weekInt){
        case 1:
            week = "星期一";
            break;
