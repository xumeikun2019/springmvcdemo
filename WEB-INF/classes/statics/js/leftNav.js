function navBar(strData){
	...
	if(typeof(strData) == "string"){
		var data = JSON.parse(strData); //部分用户解析出来的是字符串，转换�?�?
	}else{
		data = strData;
	}	
			...
			for(var j=0;j<data[i].children.length;j++){
                ...
                if(data[i].children[j].children != undefined && data[i].children[j].children.length > 0){
                	...
                	for(var k=0;k<data[i].children[j].children.length;k++){
                        ulHtml += '<dd><a href="javascript:;" data-url="'+data[i].children[j].children[k].href+'"><cite>&nbsp;&nbsp;&nbsp;'+data[i].children[j].children[k].title+'</cite></a></dd>';
                    }
                    ulHtml += "</dl>";
                }
