...
//    console.log(signList);
  ...
  draw:function(signList,signLists){
    ...
    $("#calendar1").html(str);
    //绑定日历表头
    var calendarName=calUtil1.showYear+"年"+calUtil1.showMonth+"月";
  ...
  drawCal : function(iYear, iMonth ,signList,signLists) {
   ...
   htmls.push("</div>");
   htmls.push("<div>");
   htmls.push("<div style='display:flex;justify-content:space-around;margin-bottom:0px;border-left: 1px solid #e6e6e6;border-right: 1px solid #e6e6e6'><div>星期日</div><div>星期一</div><div>星期二</div><div>星期三</div><div>星期四</div><div>星期五</div><div>星期六</div></div>");
   htmls.push("</div>");
